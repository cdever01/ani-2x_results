...
F = []
for x in X:
    coordinates = torch.tensor(x,requires_grad=True,device=device).unsqueeze(0)
...
f, ax = plt.subplots(2, sharex=True,figsize=(10, 5), dpi= 300, facecolor='w', edgecolor='k')
...
errorstr = "MAE:   " + "{0:.2f}".format(Emae) + "\nRMSE: " + "{0:.2f}".format(Erms)

ax[0].plot(t,HARTREE_TO_KCALMOL*Edf,'-',color='black',linewidth=1,label=r'$E_{DFT}$')
ax[0].plot(t,HARTREE_TO_KCALMOL*Era,'-',color='green',linewidth=1,label=r'$E_{ANI}$')
ax[0].plot(t,HARTREE_TO_KCALMOL*np.abs(Era-Edf),'-',color='red',linewidth=1,label=r'$\left|E_{ANI}-E_{DFT}\right|$')
ax[0].set_xlim([0,t[-1]+0.001+6.0])
ax[0].axvline(25.0,color='black',linewidth=0.8)
ax[0].text(0.845, 0.48, errorstr, transform=ax[0].transAxes, fontsize=12, verticalalignment='top', bbox=props)
...
Frms = HARTREE_TO_KCALMOL*calculaterootmeansqrerror(F.flatten(),Fdft.flatten())
errorstr = "MAE:   " + "{0:.2f}".format(Fmae) + "\nRMSE: " + "{0:.2f}".format(Frms)

ax[1].plot(t,HARTREE_TO_KCALMOL*(np.mean(np.abs(Fdft),axis=1)),'-',color='black',linewidth=1,label=r'$\left<\left|\left|F_{DFT}\right|\right|\right>$')
ax[1].plot(t,HARTREE_TO_KCALMOL*(np.mean(np.abs(F),axis=1)),'--',color='green',linewidth=1,label=r'$\left<\left|\left|F_{ANI}\right|\right|\right>$')
ax[1].plot(t,HARTREE_TO_KCALMOL*(np.mean(np.abs(F-Fdft),axis=1)),'-',color='red',linewidth=1,label=r'$\left<\left|\left|F_{ANI}-F_{DFT}\right|\right|\right>$')
ax[1].axvline(25.0,color='black',linewidth=0.8)
ax[1].text(0.845, 0.29, errorstr, transform=ax[1].transAxes, fontsize=12, verticalalignment='top', bbox=props)
...
newax.imshow(im)
newax.text(0.49, 0.83, "GSK1107112A", transform=newax.transAxes, fontsize=12, verticalalignment='center', horizontalalignment='center')
newax.set_xticks([])
newax.set_yticks([])
...
print(HARTREE_TO_KCALMOL*calculatemeanabserror(F.flatten(),Fdft.flatten()),HARTREE_TO_KCALMOL*calculaterootmeansqrerror(F.flatten(),Fdft.flatten()))
