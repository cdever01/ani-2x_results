...
def r2_and_sp(x, ys):
    ...
    for y in range(len(ys)):
        slope, intercept, r_value, p_value, std_err = scipy.stats.linregress(x, ys[y])
...
#    ANI_at_ANI_conf-=min(ANI_at_ANI_conf)
#    ANI_at_DFT_conf-=min(ANI_at_DFT_conf)
#    DFT_at_DFT_conf-=min(DFT_at_DFT_conf)
#    DFT_at_ANI_conf-=min(DFT_at_ANI_conf)

    di=all_diffs([ANI_at_ANI_conf, ANI_at_DFT_conf, DFT_at_DFT_conf, DFT_at_ANI_conf])
    for ANI_at_ANI, ANI_at_DFT, DFT_at_DFT, DFT_at_ANI in zip(di[0], di[1], di[2], di[3]):
