...
for name in names:
    for i in range(len(conf_data['Structure'])):
        if name in conf_data['Structure'][i]:
            ...
            db, p1, p2, r = geom.comp(mpf, df, 'bond')
            ab, p1, p2, r = geom.comp(mpf, af, 'bond')
            pb, p1, p2, r = geom.comp(mpf, pf, 'bond')
            mb, p1, p2, r = geom.comp(mpf, mf, 'bond')
            
            da, p1, p2, r = geom.comp(mpf, df, 'ang')
            aa, p1, p2, r = geom.comp(mpf, af, 'ang')
            pa, p1, p2, r = geom.comp(mpf, pf, 'ang')
            ma, p1, p2, r = geom.comp(mpf, mf, 'ang')
            
            dd, p1, p2, r = geom.comp(mpf, df, 'dih')
            ad, p1, p2, r = geom.comp(mpf, af, 'dih')
            pd, p1, p2, r = geom.comp(mpf, pf, 'dih')
            md, p1, p2, r = geom.comp(mpf, mf, 'dih')
...
Dihsm=[dft_dihedral_mae, ani_dihedral_mae, pm6_dihedral_mae, mmff_dihedral_mae]
RMSDs=[np.mean(dft_rmsd), np.mean(ani_rmsd), np.mean(pm6_rmsd), np.mean(mmff_rmsd)]
gdata={}
